...
public abstract class AbstractItemUpgradeTableScreen<T extends AbstractItemUpgradeTableScreenHandler> extends HandledScreen<T> implements RecipeBookProvider {
    private static final Identifier RECIPE_BUTTON_TEXTURE = new Identifier("textures/gui/recipe_button.png");
    public final AbstractItemUpgradeTableRecipeBookScreen recipeBook;
    ...
    public void init() {
        ...
        this.x = this.recipeBook.findLeftEdge(this.narrow, this.width, this.backgroundWidth);//将主gui界面弄到旁边，让位给recipebook
        this.addButton(new TexturedButtonWidget(this.x + 20, this.height / 2 - 47, 20, 18, 0, 0, 19, RECIPE_BUTTON_TEXTURE, (buttonWidget) -> {
            this.recipeBook.reset(this.narrow);
            this.recipeBook.toggleOpen();
            this.x = this.recipeBook.findLeftEdge(this.narrow, this.width, this.backgroundWidth);
            ((TexturedButtonWidget) buttonWidget).setPos(this.x + 20, this.height / 2 - 47);
        }));
        this.titleX = (this.backgroundWidth - this.textRenderer.getWidth((StringVisitable) this.title)) / 2;//标题居中
    }
    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
      ...
      /*if (((AbstractItemUpgradeTableScreenHandler)this.handler).isBurning()) {
        ...
        l = ((AbstractItemUpgradeTableScreenHandler) this.handler).getUpgradeProgress();
        this.drawTexture(matrices, i + 84, j + 42, 0, 166, l + 1, 6);
